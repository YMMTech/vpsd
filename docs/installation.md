# Install Simploy and deploy with GitLab

This guide takes one Next.js application from a fresh project directory to HTTPS on a Debian or Ubuntu VM. Install the published Simploy package from the registry. The optional [Run locally](#run-locally) section is for contributors testing source builds.

Commands are labeled by where they run. Use a Bash-compatible terminal on your local computer. Simploy prepares the VM and generates project files; the generated GitLab pipeline performs deployments afterward.
...
- Git and an OpenSSH client locally, and Git available to `simploy init`.
...
The walkthrough uses deployment user `simploy`, SSH port `22`, application port `3000`, deployment directory `/home/simploy/app`, and Caddy configuration `/etc/caddy/Caddyfile`. For another SSH port, replace `22` in all SSH commands and set the same `VPS_PORT` in GitLab.

## 2. Install Simploy and prepare the VM
...
id simploy
sudo docker network inspect simploy-ingress
```
...
```bash
DEPLOY_USER=deployer SIMPLOY_CADDY_CONFIG_PATH=/etc/caddy/custom/Caddyfile simploy setup
getent passwd deployer
id -gn deployer
```

Use the reported home directory and primary group in the SSH installation commands below. Set `VPS_USER=deployer` and a writable `SIMPLOY_DEPLOY_PATH`, such as `/home/deployer/app`, in GitLab. The deployment directory default does not change with `VPS_USER`.

Set the same `SIMPLOY_CADDY_CONFIG_PATH` in GitLab and ensure the persistent Caddy service uses that configuration path. Setup authorizes the path for CI; it does not rewrite Caddy's systemd service to use an arbitrary custom path. Keep the standard path unless you already manage a custom configuration. A custom deployment directory outside the user's home must be created and made writable by the admin beforehand.
...
mkdir my-app
cd my-app
simploy init --name my-app --app nextjs --ci gitlab \
  --services none --domain app.example.com --port 3000 --app-default
```

Expect `Simploy project initialized.` The project root contains `.git/`, `app/`, `.gitlab-ci.yml`, and `simploy/` with `compose.yml`, `Caddyfile`, and `deploy.env`. Simploy initializes Git itself; attach the GitLab remote afterward, in step 8. Do not clone an existing repository into this directory before init.
...
```bash
ssh-keygen -t ed25519 -f ~/.ssh/simploy_gitlab -C "simploy-ci"
cat ~/.ssh/simploy_gitlab.pub
```
...
No password for `simploy` is needed. Do not start with `ssh-copy-id` unless that account already has an authentication method. Keep the private key on your local computer and in GitLab's CI variable; never copy it to the VM or commit it.
...
```bash
ssh-keyscan -p 22 -t ed25519 YOUR_VM_IP > ~/.ssh/simploy_known_hosts
ssh-keygen -lf ~/.ssh/simploy_known_hosts
```
...
```bash
ssh -p 22 -i ~/.ssh/simploy_gitlab \
  -o IdentitiesOnly=yes -o BatchMode=yes \
  -o StrictHostKeyChecking=yes \
  -o UserKnownHostsFile="$HOME/.ssh/simploy_known_hosts" \
  simploy@YOUR_VM_IP 'id; docker ps; docker compose version'
```
...
**GitLab UI:** open the project's **Settings > CI/CD > Variables** and add the following. The generated Simploy pipeline expects **Type: Variable**, including for both SSH values. It writes their contents into files itself; File-type variables would supply temporary filenames instead of the key contents.

| Key | Value |
| --- | --- |
| `VPS_HOST` | `YOUR_VM_IP`, with no protocol, username, or port |
| `VPS_USER` | `simploy`, or your custom deployment account |
| `SSH_PRIVATE_KEY` | Entire local `~/.ssh/simploy_gitlab` private key, including BEGIN/END lines and a final newline |
| `SSH_KNOWN_HOSTS` | Entire verified local `~/.ssh/simploy_known_hosts` contents |

**Local computer:** display the values for copying into GitLab, without sharing the private-key output in logs or messages:

```bash
cat ~/.ssh/simploy_gitlab
cat ~/.ssh/simploy_known_hosts
```

Use literal multiline contents, not quoted strings or `\n` escapes. Disable variable-reference expansion. Multiline SSH keys cannot meet GitLab's masking restrictions; use **Visible** for the SSH values and never print them in CI. Protect deployment variables and ensure the default branch is protected. Use environment scope `*` for this walkthrough. GitLab documents these controls in [CI/CD variables](https://docs.gitlab.com/ci/variables/) and [SSH keys](https://docs.gitlab.com/ci/jobs/ssh_keys/); its generic File-variable example differs from Simploy's generated contract.
...
| `SIMPLOY_DEPLOY_PATH` | `/home/simploy/app` | Custom deployment user or directory |
| `SIMPLOY_CADDY_CONFIG_PATH` | `/etc/caddy/Caddyfile` | Same custom path authorized during setup and used by Caddy |
...
**Local computer, generated project root:** check `simploy/deploy.env` contains your actual domain and port:
...
Keep only those two non-secret values in this committed file. Review `simploy/Caddyfile`, `simploy/compose.yml`, `app/Dockerfile`, and `.gitlab-ci.yml`. CI replaces the configured Caddyfile with the generated one; this walkthrough assumes a VM dedicated to this single application.
...
If `.gitlab-ci.yml` conflicts and you want to keep the newly generated Simploy pipeline:
...
For later deployments, edit the application, review and commit the changes, and push to the default branch normally. GitLab builds a new image and deploys its digest; rerunning `simploy init` or `simploy setup` is not part of routine application deployment.
...
| `Permission denied (publickey)` or password prompt | **Local:** use the exact key and user in step 5. **VM admin:** inspect `sudo ls -ld /home/simploy/.ssh` and `sudo ls -l /home/simploy/.ssh/authorized_keys`; expect owner `simploy`, modes 700/600, and the matching single-line public key. Correct using step 4. Do not enable password login as a workaround. |
...
| Caddy sudo asks for a password | **VM admin:** inspect `sudo cat /etc/sudoers.d/simploy-caddy`. Match the deployment user and configuration path to CI. Generated CI uses `/usr/bin/install`, `/usr/bin/env`, and `/usr/bin/caddy`; rerun setup with the intended settings if permissions differ. Do not grant unrestricted passwordless sudo. |
...
**Local computer, Simploy source repository:**
...
Build before packing: the current package has no `prepack` build hook. Use the actual packed filename printed by `pnpm pack`; the example below uses version `0.1.0`.
...
```bash
SIMPLOY_CHECK_DIR="$(mktemp -d)"
cd "$SIMPLOY_CHECK_DIR"
simploy init --name template-check --app none --ci gitlab \
  --services none --domain test.example.com --port 3000
head -20 .gitlab-ci.yml
```

Expect the corrected GitLab verification block. This checks generation, not a Next.js build or VM deployment. Invoking `node dist/cli/index.js` directly only checks the source build; it does not verify what the global `simploy` command runs.

## Manual validation record

The previous walkthrough confirmed the public-key installation method, reproduced an outdated globally installed template, and verified the corrected source-build output. The user subsequently reported everything worked after reinstalling the local tarball under a unique name. Individual final pipeline and HTTPS results were not recorded.

The documentation has been checked against the current setup and CI implementation. A fresh walkthrough using this published-registry guide is still pending. Record the following without including secrets:

| Check | Result |
| --- | --- |
| Date and tester | Pending |
| Published package version (`pnpm list -g --depth 0`) | Pending |
| Installation route | Registry; pending validation |
| VM OS/version (`cat /etc/os-release`) | Pending |
| Generated app / CI / services | Next.js / GitLab / none; pending validation |
| Installed `simploy init` and generated CI inspection | Pending registry walkthrough |
| Setup and key-only SSH/Docker checks | Pending registry walkthrough |
| Verification, image-build, deployment jobs | Pending |
| Public HTTPS response and browser page | Pending |
| Subsequent push/redeployment | Pending |

Do not mark the walkthrough validated until these checks have been performed. No automated tests are required for this documentation change.
